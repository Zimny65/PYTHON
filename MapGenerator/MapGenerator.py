...
def generate_map(df, condition, output_filename):
...
if __name__ == "__main__":
    ...
    df['CONDITION'] = df.iloc[:, 18].apply(lambda x: int(x) if x else None)
    ...
    conditions = [
        ("CONDITION == 1", "green_map.html"),   # Mapa dla zielonych markerów
        ("CONDITION == 0", "red_map.html"),     # Mapa dla czerwonych markerów
        ("CONDITION == CONDITION", "all_map.html")  # Mapa dla wszystkich danych
    ]

    # Iteracja po warunkach i generowanie map
    for condition, output_file in conditions:
        generate_map(df, condition, output_file)
